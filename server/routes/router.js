...
router.post("/addsubject", async (req, res) => {
  try {
    ...
    if (add) res.status(200).send({ subject: add });
...
router.post("/addexam", async (req, res) => {
  try {
    ...
    else res.status(400).send({ error: "cannot store in database" });
  } catch (error) {
    console.log(error);
    res.status(500).send({ error });
  }
...
router.get("/showresult/:id", async (req, res) => {
  try {
    ...
    if (!data) res.status(400).send({ error: "enter valid credentials" });
    res.status(200).send({ data });
  } catch (error) {
    res.status(500).send({ error });
  }
